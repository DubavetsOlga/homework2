...
function Clock() {
    ...
    const start = () => {
        ...
        const id = setInterval(() => setDate(new Date(restoreState('hw9-date', Date.now()))), 1000) as unknown as number;
        setTimerId(id)
    }
    ...
    const stringDate = `${addedFirstZero(date.getDate())}.${addedFirstZero(date.getMonth()+1)}.${date.getFullYear()}` || <br/>
